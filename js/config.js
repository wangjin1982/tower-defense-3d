...
// 敌人行走路径（格子坐标 [col,row]，从左入右出）—— 单入口经典地图
...
// 双入口合作地图：上下两个传送门，两条路径在中部汇合，共享同一个核心
export const MAP_DUAL = {
  name: '双子峡谷',
  paths: [
    [ // 上路（玩家 1 防守）
      [0, 1], [1, 1], [2, 1], [3, 1], [4, 1], [4, 2], [4, 3], [5, 3],
      [6, 3], [7, 3], [8, 3], [8, 4], [9, 4], [10, 4], [11, 4], [12, 4],
    ],
    [ // 下路（玩家 2 防守）
      [0, 7], [1, 7], [2, 7], [3, 7], [4, 7], [4, 6], [4, 5], [5, 5],
      [6, 5], [7, 5], [8, 5], [8, 4], [9, 4], [10, 4], [11, 4], [12, 4],
    ],
  ],
};
...
// ===== 地图注册表 =====
export const MAPS = {
  classic: { key: 'classic', name: '经典防线', icon: '🛡️', paths: [PATH_CELLS], desc: '单入口 · 单人/合作' },
  dual: { key: 'dual', name: '双子峡谷', icon: '👥', desc: '双入口 · 双人合作', paths: [MAP_DUAL.paths[0], MAP_DUAL.paths[1]] },
};
...
export const TOWERS = {
  ...
  // 🚑 医疗塔：不攻击敌人，持续治疗受损最严重的防御塔（dmg = 医疗值）
  medic: {
    key: 'medic', name: '医疗塔', icon: '🚑', desc: '治疗受损的防御塔',
    cost: 90, dmg: 20, range: 4.8, rate: 1.0, color: 0xff8a80, kind: 'medic', hp: 110,
  },
  // ⛑️ 战地维修：机枪塔 + 医疗塔合体产物（高速维修）
  medgun: {
